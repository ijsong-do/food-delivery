<template>
    <v-card outlined>
        <v-card-title>
            FoodCooking # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="CustomId" v-model="item.customId" :editMode="editMode" @change="change" />
            </div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/foodCookings/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
